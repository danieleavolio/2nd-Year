function xStar = fitPsvm(X, y, C)
	...
	% For v we have a 2x2 matrix since the points are in R^2
	% For GAMMA we have only a 1x1 matrix since it is a scalar
  %La matrice hessiana è composta solo dai coefficienti numerici delle X
  %Le variabili della diagonale sono le variabile quadratiche mentre quelle non quadratiche (che noi non abbiamo) sono nel resto della matrice,per questo lasciamo zero
	for i = 1: 3
		H(i, i) = 1;
	endfor
