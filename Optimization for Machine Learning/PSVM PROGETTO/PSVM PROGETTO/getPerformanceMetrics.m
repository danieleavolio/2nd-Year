function performanceMetrics = getPerformanceMetrics(X, y, vStar, gammaStar)
	% Correctness = (n points correctly classified) / (total points in the set)
	correctness = getNumCorrectlyClassified(X, y, vStar, gammaStar) / size(X)(1);
	...
	for i = 1: length(y)
		if y(i) == 1 % if the point is labeled as positive...
			...
			if dot(vStar', X(i, :), 2) >= gammaStar % ... and the model puts it in A+... la funzione dot poteva essere scritta anche cosi vStar * X(i),è uguale
	...
	%disp(strcat("First level: numCorrClass A: ", num2str(numCorrectlyClassifiedA)));
	%disp(strcat("First level: numCorrClass B: ", num2str(numCorrectlyClassifiedB)));
